...
with col1:
    with st.beta_expander("Locations"):
        ...
        radiusInput = st.number_input(
            "Radius around the location, in kilometers",format='%f', min_value=0.5, value=1.0, key="numberinput1"
        )
        if locationInput != "" and radiusInput > 0.5:
            with col3:
                ...
                if coordinatesList != []:
                    ...
                    folium.Circle(
                        coordinatesList, radius=radiusInput*1000, fill=True, fill_color="#3186cc"
                    ).add_to(m2)
                    for lat, lon, scene in allLocations:
                        a = haversine(coordinatesList[1], coordinatesList[0], float(lon), float(lat))
                        if a < (radiusInput):
                            folium.Marker((lat,lon),tooltip=scene).add_to(m2)
                            print(lon,lat, 'zit in de radius van', coordinatesList, 'op basis van radius', radiusInput)
                    folium_static(m2)
